...
def run_tune(model_name, model_class, data, cv_type='kfold'):
    ...
    def trainable(config):
        X_train, X_test, y_train, y_test = data
        try:
            model = train_model(config, model_class, (X_train, X_test, y_train, y_test))
            cv_scores = cross_val_score(model, X_train, y_train, cv=5, scoring="neg_mean_squared_error")
            mean_cv_score = -np.mean(cv_scores)
            tune.report(mean_cv_score=mean_cv_score)
        except Exception as e:
            print(f"Error during training: {e}")
            tune.report(mean_cv_score=float('inf'))

    dir_path = os.path.join(os.getcwd(), 'ray_results')
...
# Main function for running hyperparameter search
def main():
