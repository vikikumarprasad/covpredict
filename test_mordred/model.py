...
def display_results(model_name, X_train, X_test, y_train, y_test, y_scaler):
    ...
    plt.plot(y_train , y_train_pred , 'o' , color = "indianred" , markersize = 1 )
...
def fit_and_evaluate(model_name, model_class, data):
    ...
    rfecv = RFECV(estimator=model, step=1, cv=5, scoring='neg_mean_squared_error', verbose=3)
...
def main():
    X_train = pd.read_csv('X_train-mordred.csv')
    X_test = pd.read_csv('X_test-mordred.csv')
    y_train = pd.read_csv('y_train-mordred.csv')
    y_test = pd.read_csv('y_test-mordred.csv')
    y_scaler = 'y_scaler-mordred.pkl'
