...
with open(row_list_path, 'r') as file:
    molfile_list = [line.strip() + ".mol" for line in file if line.strip()]

with open(row_list_path, 'r') as file:
    xyzfile_list = [line.strip() + ".xyz" for line in file if line.strip()]
...
for filename, entry, xyzname in zip(molfile_list, Plist_mol, Plist_xyz):
    ...
    inpfeats.insert(0, 'Datapoint', filename_clean)
    results.append(inpfeats)

df = pd.concat(results, ignore_index=True)
print(df.head)
df.to_csv('tsfeats_mordreddesc.csv', index=False)
